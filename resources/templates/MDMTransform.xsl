...
<xsl:stylesheet version="2.0"
	...
	{
		<xsl:apply-templates
			select="mdm:TCRMService/mdm:TxResponse/mdm:ResponseObject/mdm:SearchPartyWrapperBObj"
			mode="map" />
	}
	</xsl:template>
	...
	<xsl:template name="PartyRecords" match="mdm:SearchPartyWrapperBObj" mode="map">
             ...
             "certifiedResults" : [
             	<xsl:for-each select="mdm:PartyWrapperBObj">
		 			<xsl:call-template name="CertifiedRecords"/>		 			
		 		</xsl:for-each>             	            						
		    ...
		    "probableResults" :[ 
		 		<xsl:for-each select="mdm:PartyWrapperBObj">
		 			<xsl:call-template name="ProbableRecords"/>
		 		</xsl:for-each>		 			
    ...
    <xsl:template name="ProbableRecords" match="mdm:PartyWrapperBObj" mode="map">
         			     			         			         			
            		      <xsl:if test="mdm:TCRMPersonBObj/mdm:TCRMSuspectBObj/mdm:AdjustedMatchCategoryCode = 'Probable-B'">
	                                <xsl:apply-templates select="mdm:TCRMPersonBObj" mode="map" />
	                                <xsl:if test="position() &gt; 0 and position() != last()">
                                       ,
                                    </xsl:if>
	                      </xsl:if>
	                      <xsl:if test="mdm:TCRMOrganizationBObj/mdm:TCRMSuspectBObj/mdm:AdjustedMatchCategoryCode = 'Probable-B'">
	                                <xsl:apply-templates select="mdm:TCRMOrganizationBObj"  mode="map" />
	                                <xsl:if test="position() &gt; 0 and position() != last()">
                                       ,
                                    </xsl:if>
	...
	<xsl:template name="PersonES" match="mdm:TCRMPersonBObj" mode="map">
		...
		<xsl:if test="mdm:ClientStatusValue !=''">"status": "<xsl:value-of select="mdm:ClientStatusValue" />",</xsl:if>
		<xsl:if
	   ...
	   <xsl:if test="mdm:TCRMExtension/mdm:XPersonBObjExt/mdm:XOccupationValue">
		...
		<xsl:if
			test="mdm:TCRMPartyContactMethodBObj[mdm:TCRMContactMethodBObj/mdm:ContactMethodValue='Telephone Number'] !=''">
			"phones": [
		...
		<xsl:if
			test="mdm:TCRMPartyContactMethodBObj[mdm:TCRMContactMethodBObj/mdm:ContactMethodValue='Email Address'] !=''">
			"emails": [
	...
	<xsl:template name="OrganizationES" match="mdm:TCRMOrganizationBObj" mode="map">
		...
		<xsl:if
			test="mdm:TCRMAdminContEquivBObj/mdm:AdminSystemValue !='' and mdm:TCRMAdminContEquivBObj/mdm:AdminPartyId !=''">
		"sourceSystems": [{
		"sourceSystemId": "<xsl:value-of select="mdm:TCRMAdminContEquivBObj/mdm:AdminPartyId" />",
		"sourceSystemName": "<xsl:value-of select="mdm:TCRMAdminContEquivBObj/mdm:AdminSystemValue" />"
			}]
